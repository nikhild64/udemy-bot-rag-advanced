...
function ChatExperience() {
  ...
  useEffect(() => {
    if (initialPrompt && isLoaded && !hasExecutedRef.current) {
      ...
      <header className="sticky top-0 z-10 bg-background/80 backdrop-blur-md border-b border-border">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <a aria-label="ChaiCode home" className="active" href="/" data-status="active" aria-current="page">
              <div className="flex items-center gap-0.5 font-onest text-xl font-medium tracking-tight">
                <div style={{ transform: "rotate(3.62886deg)" }}>
                  <svg viewBox="0 0 2000 2000" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" className="-ml-1.5 size-7.5 fill-[#B0B0B0] stroke-[#B0B0B0] dark:fill-[#ffffff] dark:stroke-[#ffffff]">
          ...
          <div className="flex items-center gap-4">
            ...
            {isLoaded && !isSignedIn && (
              <SignInButton mode="modal">
                <button className="px-4 py-2 rounded-md bg-primary text-primary-foreground font-medium text-sm hover:bg-primary/90 transition-colors">
                  Sign In
                </button>
              </SignInButton>
            )}
        ...
        {!response && !isStreaming && !error ? (
          ...
          <div className="w-full max-w-6xl mx-auto flex flex-col gap-8">
            ...
            <div className="w-full flex flex-col gap-6">
              ...
              {error && (
                <div className="w-full p-4 rounded-xl border border-destructive/20 bg-destructive/10 text-destructive flex items-center gap-3">
                  <ServerCrash className="w-5 h-5 shrink-0" />
                  <p className="font-medium text-sm">An error occurred, please try again later.</p>
                </div>
              )}
            </div>

            {/* Bottom Row: Answer & Timeline */}
