...
export function DashboardView({
  ...
  return (
    <>
      ...
      `}} />
      <div className="dash-view p-4 lg:py-5 lg:px-6 space-y-4 lg:space-y-5 max-w-7xl mx-auto h-full w-full overflow-y-auto">
        {/* Header Banner */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 border-b border-[#2B2B2B] pb-4">
          <div>
            <h1 className="text-xl lg:text-2xl font-bold text-white tracking-tight flex items-center gap-2">
              <Sparkles className="h-5 w-5 lg:h-6 lg:w-6 text-[#F2A23A]" />
          ...
          <div className="flex items-center gap-2.5">
            <button
              ...
              className="dash-btn-outline px-3 py-1.5 text-xs flex items-center gap-2"
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5">
          <div className="dash-card p-3.5 lg:p-4 relative overflow-hidden">
            <div className="flex items-center justify-between">
              <span className="text-xs font-medium text-[#A9A9A9]">{t('dashboard.stats.notebooks', 'Notebooks')}</span>
              <div className="dash-icon-box p-1.5">
                <BookOpen className="h-3.5 w-3.5" />
              </div>
            </div>
            <div className="mt-2">
              <span className="text-xl lg:text-2xl font-bold text-white">{stats.totalNotebooks}</span>
          ...
          <div className="dash-card p-3.5 lg:p-4 relative overflow-hidden">
            ...
            <div className="mt-2">
              <span className="text-xl lg:text-2xl font-bold text-white">{stats.totalSources}</span>
          ...
          <div className="dash-card p-3.5 lg:p-4 relative overflow-hidden">
            ...
            <div className="mt-2">
              <span className="text-xl lg:text-2xl font-bold text-white">{formatSize(stats.totalStorageBytes)}</span>
            </div>
          </div>
          ...
          <div className="dash-card p-3.5 lg:p-4 relative overflow-hidden">
            ...
            <div className="mt-2 flex items-center justify-between">
              <div>
                <span className="text-xl lg:text-2xl font-bold text-white">{stats.activeJobsCount}</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                {data.recentNotebooks.map((nb: any) => (
                  <div
                    ...
                    className="dash-card p-3.5 lg:p-4 hover:bg-[#232323]/50 transition cursor-pointer group flex flex-col justify-between"
                  >
                    <div>
                      <div className="flex items-center justify-between">
                        <h3 className="text-xs sm:text-sm font-semibold text-white group-hover:text-[#F2A23A] transition truncate">
                          {nb.title}
                        </h3>
                        {nb.isFavorite && <Star className="h-3.5 w-3.5 text-[#F2A23A] fill-[#F2A23A] shrink-0" />}
                      </div>
                      {nb.description && (
                        <p className="text-[11px] text-[#A9A9A9] mt-1.5 line-clamp-2 leading-relaxed">{nb.description}</p>
                      )}
                    </div>

                    <div className="mt-3 pt-2.5 border-t border-[#2B2B2B] flex items-center justify-between text-[11px] text-[#A9A9A9]">
                      <span>{nb._count?.sources || 0} sources • {nb._count?.messages || 0} chats</span>
                      <ArrowUpRight className="h-3.5 w-3.5 text-[#A9A9A9] group-hover:text-[#F2A23A] transition" />
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="space-y-4 lg:space-y-5">
            ...
            <div className="dash-card p-3.5 lg:p-4 space-y-3">
              ...
              {!data?.recentSources || data.recentSources.length === 0 ? (
                <p className="text-xs text-[#A9A9A9] text-center py-4">No sources uploaded yet.</p>
              ) : (
                <div className="space-y-2">
                  {data.recentSources.map((src: any) => (
                    <div key={src.id} className="p-2 sm:p-2.5 rounded-xl bg-[#121212] border border-[#2B2B2B] flex items-center justify-between">
                      <div className="min-w-0 pr-2">
                        <p className="text-xs font-medium text-white truncate">{src.displayName || src.title}</p>
                        <p className="text-[10px] text-[#A9A9A9]">{src.notebook?.title || 'Notebook'}</p>
                      </div>
                      <span className="px-1.5 py-0.5 rounded-md bg-[#232323] text-[9px] font-mono text-[#F2A23A] shrink-0">
                        {src.status}
                      </span>
                    </div>
                  ))}
            ...
            <div className="dash-card p-3.5 lg:p-4 space-y-2.5 border-[#F2A23A]/30">
              <h4 className="text-[10px] font-semibold text-[#F2A23A] uppercase tracking-wider">Platform Operations</h4>
              <button
                onClick={() => setCleanupModalOpen(true)}
                className="w-full p-2.5 rounded-xl bg-[#121212] hover:bg-[#232323] border border-[#2B2B2B] text-left transition flex items-center justify-between text-xs text-white cursor-pointer"
              >
                <span className="flex items-center gap-2">
                  <Trash2 className="h-3.5 w-3.5 text-[#F2A23A]" /> Resource Cleanup
                </span>
                <RefreshCw className="h-3 w-3 text-[#A9A9A9]" />
              </button>
            </div>
          </div>
        </div>
